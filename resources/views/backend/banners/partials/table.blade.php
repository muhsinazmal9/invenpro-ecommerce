...
<script>
        ...
        let buildHTML = `
                <div class="">
                    <table  class="table-bordered p-5 mx-auto" style="border-color:#00000052 !important; width: 100%">
                        ...
                        <tr class="mt-5">
                            <th class="p-2">{{ __('app.link') }}</th>
                            <td class="p-2">${banner['link']}</td>
                        </tr>
                        @if (request()->input('type') == 'popup')

                        <tr class="mt-5">
                            <th class="p-2">{{ __('app.countdown_start') }}</th>
                            <td class="p-2">${new Date(banner['countdown_start']).toLocaleString('en-US')}</td>
                        </tr>
                        <tr class="mt-5">
                            <th class="p-2">{{ __('app.countdown_end') }}</th>
                            <td class="p-2">${new Date(banner['countdown_end']).toLocaleString('en-US')}</td>
                        </tr>
                        @endif
                        <tr class="mt-5">
                            <th class="p-2">{{ __('app.status') }}</th>
                            <td class="p-2">${banner['status'] == '1' ? "<span class='main-btn success-btn-light btn-hover btn-sm'>{{ __('app.enabled') }}</span" : "<span class='main-btn danger-btn-light btn-hover btn-sm'>{{ __('app.disabled') }}</span"}</td>
                        </tr>
                    </table>
                </div>`;
    ...
    function statusUpdate(slug, btn) {
        ...
        Swal.fire({
            ...
            text: "{{ __('app.you_want_to_change_the_status_of_this_card') }}",
            icon: 'warning',
